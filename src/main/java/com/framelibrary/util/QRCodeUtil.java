...
public class QRCodeUtil {
     ...
     * @param filePath  用于存储二维码图片的文件路径  @return 生成二维码及保存文件是否成功
     */
    public static boolean createQRImage(String content, int widthPix, int heightPix,
                                        Bitmap logoBm, String filePath, String fileName) {
        try {
            if (content == null || "".equals(content)) {
                return false;
            }

            filePath = FileUtil.getStorageDirectory(filePath, fileName);
    ...
    public static Bitmap createBarCode(String content) throws WriterException {
        ...
        BitMatrix matrix = new MultiFormatWriter().encode(content,
                BarcodeFormat.CODE_128, 700, 150);
